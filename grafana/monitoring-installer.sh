#!/bin/bash
# Does a multi-step installation of Prometheus, Grafana, node_exporter and Lux dashboards
...
usage () {
  ...
  echo "   --4      Step 4: Installs LuxGo Grafana dashboards"
  echo "   --5      Step 5: (Optional) Installs additional dashboards"
  echo ""
  echo "Run without any options, script will download and install latest version of LuxGo dashboards."
}
...
install_prometheus() {
  echo "LuxGo monitoring installer"
  echo "--------------------------------"
  echo "STEP 1: Installing Prometheus"
  echo
  get_environment
  check_reqs
  mkdir -p /tmp/monitoring-installer/prometheus
  cd /tmp/monitoring-installer/prometheus
  ...
  echo "https://docs.lux.network/nodes/maintain/setting-up-node-monitoring#grafana"
  echo
  echo "Reach out to us on https://chat.lux.network if you're having problems."
...
install_grafana() {
  ...
  echo "https://docs.lux.network/nodes/maintain/setting-up-node-monitoring#exporter"
  echo
  echo "Reach out to us on https://chat.lux.network if you're having problems."
...
install_exporter() {
  ...
  echo
  get_environment
  mkdir -p /tmp/monitoring-installer/exporter_archive
  cd /tmp/monitoring-installer/exporter_archive
  ...
  tar xvf node_exporter.tar.gz -C /tmp/monitoring-installer/exporter_archive --strip-components=1
  sudo mv /tmp/monitoring-installer/exporter_archive/node_exporter /usr/local/bin
  ...
  echo "https://docs.lux.network/nodes/maintain/setting-up-node-monitoring#dashboards"
  echo
  echo "Reach out to us on https://chat.lux.network if you're having problems."
}
...
install_dashboards() {
  ...
  if test -f "/etc/grafana/grafana.ini"; then
    ...
    echo "https://docs.lux.network/nodes/maintain/setting-up-node-monitoring"
    echo
    usage
    exit 0
  fi

  if test -f "/etc/grafana/provisioning/dashboards/lux.yaml"; then
  ...
  echo
  echo "Downloading..."
  mkdir -p /tmp/monitoring-installer/dashboards-install
  cd /tmp/monitoring-installer/dashboards-install

  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/c_chain.json
  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/database.json
  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/machine.json
  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/main.json
  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/network.json
  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/p_chain.json
  wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/x_chain.json
  if test -f "/etc/grafana/dashboards/subnets.json"; then
    wget -nd -m -nv https://raw.githubusercontent.com/luxfi/monitoring/master/grafana/dashboards/subnets.json
  fi
  ...
  if [ "$provisioningDone" = "false" ]; then
    ...
    sudo cp lux.yaml /etc/grafana/provisioning/dashboards/
  ...
  echo "Done!"
  echo
  echo "LuxGo Grafana dashboards have been installed and updated."
